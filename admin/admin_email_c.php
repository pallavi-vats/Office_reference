<?php
// 🔻 Attachment Download – placed at top to avoid header issues
...
function get_body_and_attachments($imap, $email_number, $structure, $part_number = '') {
    ...
    if (isset($structure->parts)) {
        ...
        if ($structure->type == 0) {
        ...
        } elseif ($structure->type >= 2 && $structure->type <= 6) {
            ...
            if (isset($structure->parameters)) {
                foreach ($structure->parameters as $param) {
                    if (strtolower($param->attribute) == 'name' && $filename === 'unknown_attachment') {
            ...
            if ($filename === 'unknown_attachment' || empty($filename)) {
                ...
                <div class="card-body">
                    ...
                    if (mysqli_num_rows($attachments_res) > 0) {
                        ...
                        while ($attach = mysqli_fetch_assoc($attachments_res)) {
                            echo '<li><a href="admin_email.php?download=' . intval($attach['id']) . '&email_id=' . intval($row['id']) . '">' . htmlspecialchars($attach['filename']) . '</a></li>';
                        }
                        echo '</ul></div>';
                    }
